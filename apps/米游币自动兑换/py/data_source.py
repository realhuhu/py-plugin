...
root_path = Path(os.path.dirname(os.path.abspath(__file__))) / "data"
...
def save_exchange_info(user_id, state):
    ...
    path = root_path / 'myb_exchange' / f'{t}.json'
...
async def exchange_action(info):
    ...
    path = root_path / 'myb_exchange' / f"{info['user_id']}-{info['商品']['id']}.json"
    path.unlink()


def get_exchange_info(user_id):
    result = ''
    path = root_path / 'myb_exchange'
    path.mkdir(parents=True, exist_ok=True)
    i = 1
    for exchange_data in path.iterdir():
        if exchange_data.name.startswith(user_id):
...
def delete_exchange_info(user_id, task_id):
    path = root_path / 'myb_exchange'
    path.mkdir(parents=True, exist_ok=True)
    current = 1
    task_id = int(task_id)
    for exchange_data in path.iterdir():
        file_name = exchange_data.name
        if file_name.startswith(user_id) and current == task_id:
            scheduler.remove_job(file_name.replace(".json", ""))
...
def _():
    ...
    for exchange_data in path.iterdir():
        info = load_json(path=exchange_data)
        t = exchange_data.name.replace(".json", "")
        scheduler.add_job(
            id=t,
            replace_existing=True,
            misfire_grace_time=5,
